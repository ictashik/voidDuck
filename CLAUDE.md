...
An offline digital desk pet. A 2D duck on an old Android phone (root optional), propped in landscape, plugged in 24/7. It watches the user via the front camera (ML Kit, fully local), reacts with Rive animations, and manages its own screen brightness and inference rate so it can sit on a desk indefinitely.
...
**You cannot test this application.** You have no camera, no device, no face. Everything that matters about this project — whether the gaze feels natural, whether the wake animation lands, whether the duck reads as alive — is invisible to you. `flutter build` succeeding tells you almost nothing.
...
**Changed:** Gaze now runs through a damped spring instead of driving Rive directly.

**Test this:**
- Move your head side to side slowly. Does the duck trail you naturally or does it feel sluggish?
...
- Active Rive state machine input values
...
**If you find yourself writing a numeric literal that affects how the duck feels, it belongs in this panel.**
...
| Interest meter | Decays when the scene is static, refreshes on novelty; low interest = duck looks away and does its own thing |
...
- **Staring contest** — sustained eye contact without blinking makes the duck fidget, then look away first. The user can win.
- **Debugging listen** — sustained close proximity (as opposed to a sudden lean-in, which startles) shifts the duck into a listening pose: settles, tilts head, slow nods. This is the thematic heart of the project.
- **Staged sleep** — eyelids droop, head nods forward, jerks back up, droops again, out. Never a cut.
- **Exit glance** — when the face leaves frame, the duck keeps watching the edge it left through, then searches, then gives up.
...
3. **Placeholder tracking** — a plain circle on screen following the face, with smoothing. Proves the whole pipeline end to end without needing any art. **Do not wait for the Rive asset to start this.**
4. **PetState machine** — the five states, brightness ramps, inference throttling. Testable with the circle.
5. **Rive integration** — swap the circle for the real duck rig, wire all inputs.
...
If you think a request will make the duck feel *less* alive, say so before implementing it. Being useful here means having an opinion about the character, not just executing instructions.