# Project Specification: VoidDuck (Desk Pet Companion) — v5

*v5 changes from v4: adds two ambient timers — lapSeconds (since the last Waking, in-memory only) and totalSeconds (lifetime, persisted). Encoded visually as a lap ring around each eye socket and a slowly-seeded background star field, never as a literal digital readout on the face. See the new subsection under Section 6, and the persistence carve-out amendment in CLAUDE.md.*
...
### Lap/Total Timers (New)

Two timers, read off `PetState` but not driving it — this feature is purely observational and ambient, never a literal digital clock on the face (that would undermine the minimalist eyes-only design).

- **lapSeconds:** elapsed time since the most recent `Waking` transition. Increments while in `Tracking` or `Idle` (the "present-ish" states); paused during `Dimming`/`Sleeping`/`Waking` itself. Resets to 0 exactly when a `Waking` transition fires. In-memory only — a fresh lap after every app restart is fine.
- **totalSeconds:** cumulative time spent in `Tracking`/`Idle` across the app's entire lifetime. Persists across restarts (see the persistence carve-out, CLAUDE.md). Increment continuously; write-through to storage roughly every 30s, or on any `PetState` transition, whichever comes first — so a crash doesn't lose much.

**Visual encoding:**
1. **Lap ring** — a thin arc around each eye socket, filling clockwise as `lapSeconds` progresses toward a reference duration (default 25 minutes, tunable). On completing a full ring: a small pulse/flash, then restart from zero — so a long session shows the ring cycling multiple times rather than sitting maxed out and static.
2. **Total-time star field** — dim pixel dots slowly seeded into the black background as `totalSeconds` accumulates, roughly one new star per some tunable number of cumulative minutes, capped at a sensible max count. Rendered well behind and dimmer than the eyes — they must never compete with the eyes as the visual focal point.
3. **Exact numbers on demand** — lap/total as HH:MM:SS are available on the same hidden debug-tap surface as the rest of the debug overlay (triple-tap, top-left): small, low-opacity monospace, off by default. This is for when the actual number is wanted, not the ambient vibe.

Like the other aliveness layers, this is independently toggleable in the debug panel.
...
- **`totalSeconds` is not a log:** the one persisted timer (see Section 6, "Lap/Total Timers") is a single monotonically increasing number — cumulative presence seconds, all-time. It is not a session log, not a timestamped history, and carries no record of *when* any individual session happened. This satisfies "no logs written to disk" in spirit as well as in letter, and should stay that way — do not expand it into anything with a per-session or per-timestamp shape.
